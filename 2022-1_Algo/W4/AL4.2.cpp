...
void roundup_carry(LargeInteger &v)
{
    ...
    {
        //앞에서 계산해서 위에 올림할 숫자 남아있으면 v[i]에 더해주기
        v[i] += carry;
        //올림수 처리
        carry = v[i] / 10;
        //나머지
    ...
    //현재 carry에 담긴 숫자는 가장 큰 자릿수까지 계산했을 때 최종적으로 남아있는 십의자리 수
    //올려줘야할 carry가 0이 아니라면 v[i] 제일 뒤에 carry 붙여주기
    if (carry != 0)
        v.push_back(carry);
}
...
void ladd(LargeInteger a, LargeInteger b, LargeInteger &c)
{
    ...
    // roundup_carry 하기 전에는 10의자리 넘는 덧셈 결과가 만들어지면
    // c[i]에 13 이런식으로 저장돼있다가
    // roundup_carry를 통해
    // c[i]=3, c[i+1]=1 로 바뀜
    //  큰 자릿수가 벡터 인덱스 뒤에 있으니까!
...
void div_by_exp(LargeInteger &u, int m, LargeInteger &v)
{
    ...
    else
    {
        ...
        copy(u.begin() + m, u.begin() + k, v.begin());
